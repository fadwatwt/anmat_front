...
function PlanTranslationsModal({ isOpen, onClose, plan }) {
      ...
      setFormData({
        ...
        feature_translations:
          existingTranslation.feature_translations?.map((ft) => ({
            ...
            title: ft.title || "",
            details: ft.details || "",
            properties: ft.properties?.map((p) => ({
              key: p.key,
              value: p.value || "",
            })) || [],
          })) || [],
  ...
  const handlePropertyChange = (featureIndex, propIndex, value) => {
    const updated = [...formData.feature_translations];
    const props = [...(updated[featureIndex].properties || [])];
    props[propIndex] = { ...props[propIndex], value };
    updated[featureIndex] = { ...updated[featureIndex], properties: props };
    setFormData((prev) => ({ ...prev, feature_translations: updated }));
  };
              ...
              {formData.feature_translations.map((ft, index) => (
                ...
                >
                  <span className="text-xs font-semibold text-primary-600 mb-2 block">
                    {ft.title || plan?.features?.[index]?.plan_feature?.title ||
                      `${t("Feature")} ${index + 1}`}
                  </span>
                  ...
                  {ft.properties && ft.properties.length > 0 && (
                    <div className="flex flex-col gap-2 mt-2">
                      <span className="text-[11px] font-medium text-cell-secondary">{t("Values")}:</span>
                      {ft.properties.map((prop, pIdx) => (
                        <div key={pIdx} className="flex items-center gap-2">
                          <span className="text-[11px] text-cell-secondary font-medium min-w-[60px]">{t(prop.key)}:</span>
                          <input
                            type="text"
                            value={prop.value}
                            onChange={(e) => handlePropertyChange(index, pIdx, e.target.value)}
                            placeholder={t("Translated value")}
                            className="flex-1 text-sm px-3 py-1.5 rounded-lg border border-status-border bg-surface text-cell-primary focus:outline-none focus:ring-1 focus:ring-primary-base"
                          />
                        </div>
                      ))}
                    </div>
