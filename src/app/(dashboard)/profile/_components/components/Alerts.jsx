    ...
    return (
        <div className={`bg-surface rounded-2xl p-6 h-full flex flex-col gap-6 ${className}`}>
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium text-cell-primary">{t("Alerts")}</h3>
                ...
                {alerts.map((alert, index) => (
                    <div key={index} className="flex items-start gap-4">
                        ...
                        <div className="flex flex-col gap-1">
                            <span className="text-sm text-cell-primary">{t(alert.title)}</span>
                            <p className="text-xs text-cell-secondary leading-relaxed text-wrap">
                                {t(alert.description)}
                            </p>
                        </div>
                    </div>
