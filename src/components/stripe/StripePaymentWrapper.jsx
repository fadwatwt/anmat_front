        ...
        fetch(`${RootRoute}/api/subscriptions/subscriber/create-setup-intent`, {
            ...
            .then((result) => {
                ...
                if (data.clientSecret) {
                    ...
                    const errMsg = typeof data.error === 'string' ? data.error : (data.error?.message || result.message || t("Failed to initialize secure checkout."));
                    setError(errMsg);
                    setLoading(false);
                } else {
                    setError(t("Failed to create a payment session."));
                    setLoading(false);
                }
            })
            .catch((err) => {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center p-10 gap-4">
                ...
                <p className="text-gray-500 font-medium text-lg dark:text-gray-400">{t("Initializing secure checkout...")}</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="p-6 bg-red-50 border border-red-200 rounded-xl text-red-600 text-center dark:bg-red-900/30 dark:border-red-800 dark:text-red-400">
                ...
                <p>{typeof error === 'string' ? error : (error.message || t("An unexpected error occurred."))}</p>
            </div>
        );
    }
