...
function InputAndLabel({title,placeholder,type,className,value ,name,onChange,isRequired = false}) {
    const {t} = useTranslation()
    return (
        <div className={`flex flex-col gap-1 w-full items-start  ${className}`}>
            <label className={"text-gray-900 dark:text-gray-200 text-sm"}>{t(title)} {isRequired && <span className={"text-red-500"}>*</span>} </label>
