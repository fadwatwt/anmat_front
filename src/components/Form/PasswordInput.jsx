...
function PasswordInput({ title, icon, id, name, onChange, onBlur, value, placeholder, isRequired = false, error }) {
    ...
    return (
        <div className={"flex flex-col items-start gap-2 w-full"}>
            ...
            <label
                className={`flex bg-status-bg pl-2 px-2 w-full items-center text-xs border-2 border-status-border rounded-xl focus-within:border-primary-400 dark:focus-within:border-primary-500 dark:focus-within:ring-primary-500 text-cell-primary ${error ? "border-red-500" : ""}`}
                htmlFor={id && ""}>
                {
                ...
                <input
                    ...
                    onChange={onChange}
                    onBlur={onBlur}
                    className={" custom-date-input text-sm bg-transparent w-full py-3 px-2 outline-none appearance-none focus:outline-none peer placeholder:text-cell-secondary/50 dark:placeholder-gray-400 " + (i18n.language === 'ar' && "text-start")}
                    name={name}
                />
