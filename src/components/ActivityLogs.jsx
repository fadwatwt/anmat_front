...
function ActivityLogs({ activityLogs, className }) {
    ...
    return (
        <div className={"flex flex-col w-full p-4 rounded-2xl items-start gap-3 bg-status-bg border border-status-border"}>
            <p className={"text-lg text-table-title"}>{t("Activity Logs")}</p>
                            ...
                            <div className={"flex flex-col justify-start items-center h-full"}>
...

                                {
                                    index < activityLogs.length - 1 && (
                                        <div className={"w-[1px] bg-status-border h-full"}></div>
                                    )}
                            </div>
                            <div className={"flex flex-col items-start gap-2 pb-4"}>
                                <div className={"flex gap-1 justify-start"}>
                                    <p className={"text-sm text-cell-primary"}>{t(activityLog.title)}</p>
                                </div>
                                <p className={"max-w-full text-wrap text-start text-xs text-cell-secondary"}>
                                    {activityLog.description}
                                </p>
                                <span className={"text-[11px] text-cell-secondary opacity-80"}>{translateTime(activityLog.timeAgo)}</span>
                            </div>
