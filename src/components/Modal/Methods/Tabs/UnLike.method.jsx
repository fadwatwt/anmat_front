...
import { useUnlikeTweetMutation } from "@/redux/socialMedia/twitterAccountsApi";

function UnLikeMethod() {
    const { t } = useTranslation();
    const [unlike, { isLoading }] = useUnlikeTweetMutation();
    ...
    const handleSubmit = async () => {
        ...
        showProcessing(t("Removing likes..."));
        try {
            await unlike({ accounts, url: url.trim() }).unwrap();
            setApiResponse({
                isOpen: true,
                status: "success",
                message: t("Likes removed from {{count}} account(s)", { count: accounts.length }),
            });
            setUrl("");
            setAccounts([]);
        } catch (error) {
