...
const Modal = ({
  ...
  return (
    <div
      className={`fixed inset-0 bg-gray-900  flex items-center overflow-hidden justify-center z-50 ${
        classNameOpacity ? classNameOpacity : "bg-opacity-50"
      }`}
      onClick={onClose}
    >
      <div
        className={` bg-white dark:bg-gray-800 rounded-xl  shadow-lg p-4 ${
          className ? className : "lg:w-1/3 md:w-8/12 sm:w-7/12 w-11/12"
        }`}
      ...
      >
        ...
        <div className={" relative flex flex-col px-1"}>
          <div
            className={
              "max-h-[70vh] overflow-hidden tab-content overflow-y-auto"
            }
          >
            {children}
          </div>
          {isBtns && (
            <div className={"flex gap-2 py-3 w-full"}>
              ...
              <DefaultButton
                onClick={onClick}
                type="submit"
                title={btnApplyTitle ? btnApplyTitle : "Apply"}
                className={
                  "bg-primary-500 font-medium dark:bg-primary-200 dark:text-black text-white"
                }
              />
            </div>
          )}
